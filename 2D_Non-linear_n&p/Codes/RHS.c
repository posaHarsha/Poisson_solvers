...
float* RHS(int n_e,int n_n,int count,int**Element_node,float*Ae,int z,int* Boundary_nodes,float** NodeCoordinate,float Vl, float Vr, float V[]){
    ...
    //(q*(Nd+(Nv*exp((-V-1.1)/(Kbt)))-(Nc*exp(V/(Kbt)))))/eo);
