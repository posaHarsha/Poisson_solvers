...
float** AssembleStiffnessMatrix(int n_e,int n_n,int count,int**Element_node, float** NodeCoordinate,int* Boundary_nodes, float*Ve){
   ...
   //Allocating Memory for the StifnessMatrix 
    ...
    S4[1] = -(S1[1] + S2[1] + S3[1]);
    S4[2] = -(S1[2] + S2[2] + S3[2]);
    M11 = -(S1[0]*S1[0] + S1[1]*S1[1] + S1[2]*S1[2])/(9*Ve[i]*Ve[i]);
    M12 = -(S1[0]*S2[0] + S1[1]*S2[1] + S1[2]*S2[2])/(9*Ve[i]*Ve[i]);
    M13 = -(S1[0]*S3[0] + S1[1]*S3[1] + S1[2]*S3[2])/(9*Ve[i]*Ve[i]);
    M14 = -(S1[0]*S4[0] + S1[1]*S4[1] + S1[2]*S4[2])/(9*Ve[i]*Ve[i]);
    M22 = -(S2[0]*S2[0] + S2[1]*S2[1] + S2[2]*S2[2])/(9*Ve[i]*Ve[i]);
    M23 = -(S2[0]*S3[0] + S2[1]*S3[1] + S2[2]*S3[2])/(9*Ve[i]*Ve[i]);
    M24 = -(S2[0]*S4[0] + S2[1]*S4[1] + S2[2]*S4[2])/(9*Ve[i]*Ve[i]);
    M33 = -(S3[0]*S3[0] + S3[1]*S3[1] + S3[2]*S3[2])/(9*Ve[i]*Ve[i]);
    M34 = -(S3[0]*S4[0] + S3[1]*S4[1] + S3[2]*S4[2])/(9*Ve[i]*Ve[i]);
    M44 = -(S4[0]*S4[0] + S4[1]*S4[1] + S4[2]*S4[2])/(9*Ve[i]*Ve[i]);
    M21 = M12;
    M31 = M13;
    M41 = M14;
    M42 = M24;
